...
void ParallelCoordinates::draw()
{
	...
	for (int i = 0; i < column_name.size(); i++)
	{
		...
		ofSetColor(0, 0, 0);
	    string_width= verdana1.stringWidth(column_name[i]->col_name);
		verdana1.drawString(column_name[i]->col_name, axis_x- string_width/2 +5, axis_y - 13 );
...
vector<vector<int*>> ParallelCoordinates::get_brushed_info()
{
	return brushed;
}
	...
	if (axis_brushed != -1) {
		...
		for (int i = 0; i < column_val[axis_brushed].size(); i++) {
			if (*column_val[axis_brushed][i] < min || *column_val[axis_brushed][i] > max) {
				*brushed[i][axis_brushed] = 0;
			}
			else {
				*brushed[i][axis_brushed] = 1;
			}
		...
		if (label_released != -1) {
			if (label_released != label_clicked) {
				...
				for (int i = 0; i < column_val[label_clicked].size(); i++) {
					*brushed[i][label_clicked] = 1;
				}
			}
		}
		else {
			float screen_width = ofGetScreenWidth();
			float screen_height = ofGetScreenHeight();
			if (args.x <= screen_width && args.x >= 0.6f*screen_width && args.y >= 0 && args.y <= 0.4f*screen_height) {
				ofNotifyEvent(stplot_label, label_clicked, this);
			}
		}
